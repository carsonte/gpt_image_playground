        ...
        <main className="min-w-0">
          ...
          {tab === 'settings' && (
            <section>
              ...
              {siteSettings && <form onSubmit={saveSiteSettings} className="rounded-2xl border border-gray-200 bg-white dark:border-white/[0.08] dark:bg-gray-900">
                <div className="border-b border-gray-100 px-5 py-4 dark:border-white/[0.06]"><h2 className="font-bold">首页信息展示</h2><p className="mt-1 text-xs text-gray-500">控制首页标题旁的公开提示与队列状态，不影响后台统计。</p></div>
                <div className="grid gap-5 p-5 lg:grid-cols-[minmax(0,1fr)_360px]">
                  <label className="text-sm font-medium">本地存储提示文字<input value={siteSettings.privacyNoticeText} onChange={(e) => setSiteSettings({ ...siteSettings, privacyNoticeText: e.target.value })} maxLength={200} className={`${fieldClass()} mt-1.5`} /><span className="mt-1.5 block text-xs font-normal text-gray-400">显示在首页顶部，最多 200 个字符。</span></label>
                  <div className="space-y-2">
                    <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-gray-200 px-4 py-3 dark:border-white/[0.08]"><span><span className="block text-sm font-medium">本地存储提示</span><span className="mt-0.5 block text-xs text-gray-400">提醒用户图片不会存入服务器</span></span><span className="relative shrink-0"><input type="checkbox" checked={siteSettings.privacyNoticeEnabled} onChange={(e) => setSiteSettings({ ...siteSettings, privacyNoticeEnabled: e.target.checked })} className="peer sr-only" /><span className="block h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-blue-600 dark:bg-white/[0.12]" /><span className="absolute left-1 top-1 h-4 w-4 rounded-full bg-white shadow transition peer-checked:translate-x-5" /></span></label>
                    <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-gray-200 px-4 py-3 dark:border-white/[0.08]"><span><span className="block text-sm font-medium">生图队列状态</span><span className="mt-0.5 block text-xs text-gray-400">显示空闲、生成中和排队数量</span></span><span className="relative shrink-0"><input type="checkbox" checked={siteSettings.queueStatusEnabled} onChange={(e) => setSiteSettings({ ...siteSettings, queueStatusEnabled: e.target.checked })} className="peer sr-only" /><span className="block h-6 w-11 rounded-full bg-gray-200 transition peer-checked:bg-blue-600 dark:bg-white/[0.12]" /><span className="absolute left-1 top-1 h-4 w-4 rounded-full bg-white shadow transition peer-checked:translate-x-5" /></span></label>
                  </div>
                </div>
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 px-5 py-4 dark:border-white/[0.06]"><span className="text-xs text-gray-400">前台约 5 秒内自动同步最新设置。</span><button type="submit" className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">保存首页设置</button></div>
              </form>}
            </section>
          )}
