...
CatAPI 承担固定 2K 主线路和可选的 4K 主线路。托管版 GPT 默认 2K，只允许用户选择 2K/4K；后台“系统设置 → GPT 生图队列”只切换 4K 的两种路由：
...
- CatAPI 的通用 `gpt-image-2` 会把部分高分辨率请求降级；服务端因此按请求尺寸自动选模：2K 使用 `CATAPI_2K_MODEL`（默认 `gpt-image-2-2k`），4K 使用 `CATAPI_4K_MODEL`（默认 `gpt-image-2-4k`）。生图和图片编辑均应用该规则，旧客户端的 1K 请求会先升级到同一比例的 2K。
...
- 由于实测 Sixoner 的 2K 请求会降到约 1254×1254，2K 请求无论后台选择哪条主线路都固定优先 CatAPI；CatAPI 不可用时再回退 Sixoner、BlackEngine。4K 遵循后台选择。
...
服务端只有在图片响应完整传输给浏览器后，才把生成记录标记为成功并释放队列槽。响应仍在传输时，后台“实时任务”继续显示为生成中；浏览器解码完成后会回报真实输出像素，后台可对比输入/输出档位并识别上游降级，浏览器写入本地 IndexedDB 的时间不计入服务器耗时。
