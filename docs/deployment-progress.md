...
- 生成记录新增实际上游线路与完整回退链、输入尺寸/档位、浏览器解码后的真实输出尺寸/档位和质量、耗时、状态和错误摘要，能够识别“请求 4K/高质量、实际上游降级”的情况。
- 代理日志同时保存请求质量、上游响应质量和 `qualityMismatch` 标记；日志只保存参数、字节数、阶段耗时和脱敏摘要，不保存图片内容、Authorization 或密钥。
- 自动测试覆盖输入图片上传、编辑动作识别、PNG 强制输出、单张输出、1K 升级、线路回退、响应体中断、真实输出参数回报、质量不一致诊断和默认质量迁移；本次发布验证为 528 项测试、服务器冒烟测试与生产构建均通过。
...
- 生成记录支持日期筛选和文字搜索。
- 生成记录保存真实 IP、提示词、动作、实际线路与回退链、输入/输出尺寸档位、请求质量、实际上游质量、数量、状态、耗时和错误摘要，不保存图片。
- 生成记录使用固定高度的独立滚动区，避免长列表拉长整个后台页面。
- 日志中心新增独立“报错记录”栏，汇总错误级别和失败状态，展示错误原因、实际线路/模型、阶段耗时、请求/响应质量、IP 标识和请求 ID；最终上游返回的 HTTP 错误正文也会写入脱敏摘要。
