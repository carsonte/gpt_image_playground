...
- 本地构建和 512 项测试已通过。
- 本地 `SENSENOVA_API_KEY` 已配置并完成两次真实 U1 生图验收：上游返回 200，服务端已将临时图片 URL 在内存中转换为 Base64 后交给浏览器，全程不落盘。
- U1 改版尚未提交 Git，也尚未发布到生产服务器；生产环境仍需单独配置 Key。
