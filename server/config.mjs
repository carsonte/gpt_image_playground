...
export const config = {
  ...
  sixoner2kModel: process.env.SIXONER_2K_MODEL?.trim() || 'gpt-image-2-2k',
  sixoner4kModel: process.env.SIXONER_4K_MODEL?.trim() || 'gpt-image-2-4k',
  catApiUrl: (process.env.CATAPI_API_URL?.trim() || 'https://catapi.cc.cd/v1').replace(/\/+$/, ''),
  catApiKey: process.env.CATAPI_API_KEY?.trim() || '',
  catApiModel: process.env.CATAPI_MODEL?.trim() || 'gpt-image-2',
  catApi2kModel: process.env.CATAPI_2K_MODEL?.trim() || 'gpt-image-2-2k',
  catApi4kModel: process.env.CATAPI_4K_MODEL?.trim() || 'gpt-image-2-4k',
